...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // ──────────────────────────────
  // API 호출 함수들 (gameHistory 관련 외의 코드는 수정하지 않음)
  ...
  // 새로 추가: 서버의 정리된 통계 데이터 호출 함수
  async function fetchStatistics(uuid) {
    const url = `/api/statistics?uuid=${uuid}`;
    try {
      const response = await fetch(url);
      if (!response.ok) throw new Error("통계 데이터를 찾을 수 없습니다.");
      const data = await response.json();
      return data;
    } catch (error) {
  ...
  // 기존의 fetchGameHistory()와 computeStatistics() 함수는 삭제
  ...
  // 4. 서버에서 정리된 통계 데이터 가져오기 및 표시
  const statistics = await fetchStatistics(uuid);
  if (statistics) {
